...
%         Y = reshape(1:ni^2,ni,ni);
%         Y = tril(Y);
%         Y = (Y+Y')-diag(sparse(diag(Y)));
%         [uu,oo,pp] = unique(Y(:));
% 
%         model.sdpbasis{i} = sparse(1:ni^2,pp+1,1);
%         model.sdpvars = model.sdpvars + ns;
%         start = start + model.K.s(i)^2;
    end
...
Infinity = 1.0E38;
pen.nvars = n;
pen.nlin = size(model.A,1);
pen.nconstr = size(model.Anonlineq,1)+size(model.Anonlinineq,1)+size(model.A,1)+size(model.Aeq,1);
