...
\caption{Evolution des états et des variables limitées du système linéarisé (avec un retour d'état)}
\label{fig:retour_etat_final_aug}
\end{figure}

	\paragraph{} Nous n'avons pas réussi à obtenir les performances spécifiées par le cahier des charges, sur $N_{tl}$ et sur la dérivée de la commande $\frac{dW_f}{dt}$. En effet, diminuer le temps de réponse du système pour limiter le dépassement sur $N_{tl}$ se traduit par une augmentation significative de l'amplitude $\frac{dW_f}{dt}$. Un compromis a été trouvé ici et correspond à un dépassement sur $N_{tl}$ de 19.8\% et une amplitude de $\frac{dW_f}{dt}$ de 814 L/h/s.
		
	\paragraph{}Si on diminue le temps de réponse le dépassement diminue également. Un autre moyen consiste à changer les saturateurs et augmenter les limites de variation. Pour respecter le cahier des charges nous devons faire une commande $H_\infty$. Cela optimise la commande en respectant son énergie.  
